...
int16_t LibI2C_Start(I2C_TypeDef* I2Cx, uint8_t address, uint8_t direction, uint8_t ack)
{
    ...
    if (LibI2C_WaitAction(I2Cx, I2C_SR1_SB)) return 1;    // timout
	...
	if (direction == I2C_SEND)
        ...
        if (LibI2C_WaitAction(I2Cx, I2C_SR1_ADDR)) return 1;    // timout
        }
	...
	if (direction == I2C_RECV)
        ...
        if (LibI2C_WaitEvent(I2Cx, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)) return 1;  // timout
        }
...
int16_t LibI2C_ReadNext(I2C_TypeDef* I2Cx)
    ...
    if (LibI2C_WaitEvent(I2Cx, I2C_EVENT_MASTER_BYTE_RECEIVED)) return -1;  // no data
